...
def profile_work(stage):
    def decorate(function):
        ...
        def measured(work, *args, **kwargs):
            metrics = {"event": "file_work_metrics", "stage": stage, "work_id": work["id"],
                       "seconds": {}, "counts": {}, "status": "error",
                       "started_at": time.time(),
                       "container": socket.gethostname(),
                       "image": os.getenv("PUFFERFS_BUILD_REVISION", "local"),
            ...
            try:
                ...
                metrics["status"] = result["status"] if isinstance(result, dict) else result
                return result
            finally:
