...
def capture():
    ...
    directory = Path('/state/embedding-limit-'+str(len(state.get('checkpoint_cases',[]))))
    directory.mkdir()
    lines = fixture_lines()
...
def recovered():
    ...
    print(f"Recovered from canonical chunks: replayed {len(replayed)} batches, exact bytes and all 257 lines verified on both APIs.",flush=True)
...
if __name__ == '__main__':
    ...
    try:
        ...
        with run.REPORT.open('a') as output:
            output.write(json.dumps({'run_id':state.get('nonce'),'phase':'embedding-limit-'+phase,'status':status,'seconds':round(time.monotonic()-started,2)})+'\n')
        print(f'embedding-limit-{phase}: {status}',flush=True)
