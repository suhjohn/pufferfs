...
| Transformation worker | N=4, I=4 | Modal: two cores / 4 GiB | Increase I when IO leaves CPU idle and measured RSS permits it; increase N when runnable work waits with the existing workers busy. Each provider batch sends one JSONL upload containing inline media. |
