"""Deploy the CPU index role: modal deploy index_app.py."""
...
app = modal.App(os.getenv("PUFFERFS_INDEX_APP_NAME", "pufferfs-index"))
...
@modal.fastapi_endpoint(method="POST", label=os.getenv("PUFFERFS_INDEX_ENDPOINT_LABEL", "pufferfs-file-index"))
def index(item: dict):
    ...
    with work_slots, closing(client("s3")) as s3, turbopuffer_client() as tp:
        return index_file(work, token, s3, os.environ["AWS_BUCKET_NAME"], tp)
