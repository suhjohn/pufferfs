...
def embedding_vectors(org_id, chunks, encode, s3, bucket, *, connect=database):
    ...
    for row in packs:
        ...
        with timed("cache_read"):
            ...
            if live is None:
                continue
            # Immutable cache bytes need no database lock while in flight.
            # If retirement deletes them first, ordinary encoding fills the miss.
            try:
                response = s3.get_object(Bucket=bucket, Key=row["object_key"], Range=f"bytes={start}-{end - 1}")
            except ClientError as error:
                if error.response.get("ResponseMetadata", {}).get("HTTPStatusCode") != 404:
                    raise
                count("cache_read_retired")
                continue
    ...
    if missing:
        ...
        with timed("cache_store"):
            s3.put_object(Bucket=bucket, Key=key, Body=bytes(pack), ContentType="application/octet-stream")
            with connect() as conn:
                live = conn.execute("""UPDATE embedding_packs SET content_hashes=%s,last_used_at=NOW()
                    WHERE object_key=%s AND retired_at IS NULL RETURNING object_key""", (missing, key)).fetchone()
                if live is None:
                    # Retirement permanently fences the directory. Publication
                    # can use the computed vectors; the late cache object is
                    # removed by the existing tombstone cleanup protocol.
                    conn.execute("""UPDATE embedding_packs SET cleanup_due_at=NOW()
                        WHERE object_key=%s AND retired_at IS NOT NULL""", (key,))
            count("cache_packs_written" if live else "cache_upload_retired")
