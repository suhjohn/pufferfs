...
project="pufferfs-embedding-batches-$$"
...
finish() {
  ...
  "${compose[@]}" logs --no-color | python3 tests/e2e/redact.py > tests/e2e/artifacts/embedding-batches.log
...
runner_started=1
driver=("${compose[@]}" run --rm --no-deps --entrypoint python e2e /e2e/embedding_batch_limits.py)
"${driver[@]}" capture
