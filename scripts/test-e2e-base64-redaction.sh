...
project="pufferfs-base64-redaction-$$"
...
finish() {
  ...
  "${compose[@]}" logs --no-color | python3 tests/e2e/redact.py > tests/e2e/artifacts/base64-redaction.log
...
runner_started=1
driver=("${compose[@]}" run --rm --no-deps --entrypoint python e2e /e2e/base64_redaction.py)
"${driver[@]}" capture
