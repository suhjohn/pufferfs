...
trap finish EXIT
"${compose[@]}" build
"${compose[@]}" up -d --wait --scale api=2 --scale collector=2 postgres aws api api-ready transform provider-relay provider-manifest-relay index reconciler collector
runner_started=1
